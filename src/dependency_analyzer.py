...
class DependencyAnalyzer:
    ...
    def print_stats(self):
        ...
        local_funcs = [
            n for n, d in self.graph.nodes(data=True) if d.get("type") == "local"
        ]
        external_funcs = [
            n for n, d in self.graph.nodes(data=True) if d.get("type") == "external"
        ]
        ...
        if external_funcs:
            ...
            external_usage = [
                (func, self.graph.out_degree(func)) for func in external_funcs
            ]
    ...
    def plot_graph(
        ...
        regular_locals = [
            n for n in local_nodes if n not in entry_points and n not in utilities
        ]
...
<body>
    ...
    <script type="text/javascript">
        ...
        var network = new vis.Network(container, data, options);
            ...
            print(
                f"Open {output_file} in your web browser to explore the graph interactively."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Python project function dependencies"
    )
    parser.add_argument("--src", default="src", help="Source directory (default: src)")
    parser.add_argument(
        "--output",
        default="dependency_graph.gexf",
        help="Output graph file (default: dependency_graph.gexf)",
    )
    parser.add_argument("--stats", action="store_true", help="Show detailed statistics")
    parser.add_argument(
        "--plot", action="store_true", help="Generate graph visualization"
    )
    ...
    parser.add_argument(
        "--no-labels", action="store_true", help="Hide node labels in plot"
    )
